...
/* ─── Utility Macros ─────────────────────────────────────────────────────── */
#define CLAMP(x, lo, hi)   ((x) < (lo) ? (lo) : ((x) > (hi) ? (hi) : (x)))   //used for explaining the range
#define ARRAY_SIZE(a)       (sizeof(a) / sizeof((a)[0])) // 'a' -->array
...
#define FAULT_NONE   0x00        // using bit flags(not using enums-> these faults are not mutually exclusive)
...
extern TIM_HandleTypeDef htim1;  //extern actually tells that given variable has been declared already
extern TIM_HandleTypeDef htim2;  //ie not to recrete the variable but use the declared one
