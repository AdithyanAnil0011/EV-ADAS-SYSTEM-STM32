...
def draw_speed(ax, speed):
    ...
    ax.set_title('SPEED', color='#90caf9',
                 fontsize=11, pad=5,
                 fontweight='bold')

    # Background ring
    theta_bg = np.linspace(np.pi*1.2, -np.pi*0.2, 300)

    ax.plot(np.cos(theta_bg),
            np.sin(theta_bg),
            color='#1e3a5c',
            linewidth=14,
    ...
    # Inner ring
    ax.plot(0.88*np.cos(theta_bg),
            0.88*np.sin(theta_bg),
            color='#12263f',
            linewidth=6,
            solid_capstyle='round')

    # Speed zones
    theta_green = np.linspace(np.pi*1.2, np.pi*0.55, 100)
    theta_yellow = np.linspace(np.pi*0.55, np.pi*0.15, 100)
    theta_red = np.linspace(np.pi*0.15, -np.pi*0.2, 100)

    ax.plot(np.cos(theta_green),
            np.sin(theta_green),
            color='#2ecc71',
            linewidth=4)

    ax.plot(np.cos(theta_yellow),
            np.sin(theta_yellow),
            color='#f1c40f',
            linewidth=4)

    ax.plot(np.cos(theta_red),
            np.sin(theta_red),
            color='#e74c3c',
            linewidth=4)
    ...
    # Tick marks
    for kmh in range(0, 201, 20):

        f = kmh / 200.0
        ang = np.pi*1.2 - f*np.pi*1.4

        tick_len = 0.18 if kmh % 40 == 0 else 0.10

        ax.plot(
            [(1.0-tick_len)*np.cos(ang), np.cos(ang)],
            [(1.0-tick_len)*np.sin(ang), np.sin(ang)],
            color='#90a4ae',
            linewidth=2
        )
